...
description: |
  ...
  diagram as a regenerated, interactive, self-contained HTML view via `keel render model` — never
...
**`keel render model [--root ROOT]`** — emits a single self-contained interactive HTML page (cytoscape) of the
...
**Generate, don't commit:** `keel render model --root . > traceability.html` then open it. Generated `*.html`
...
2. **Hand-drawing / embedding a picture in docs** — Detection: an authored diagram or pasted image
   as the "source of truth." Resolution: the authored `.sysml` model is the truth; `keel render model`
...
1. **Generate:** `keel render model [--root ROOT] > <name>.html` (absolute ROOT per §6). Hand the file to the
