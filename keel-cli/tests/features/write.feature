Feature: write API — record result and record task

  Scenario: record result creates DoDR1 for a task with no existing results
  ...
  Scenario: record result on a task with existing DoDR1 creates DoDR2
  ...
  Scenario: record result rejects an unknown task
  ...
  Scenario: record result rejects an invalid verdict
  ...
  Scenario: record task inserts action and DoD verification
  ...
  Scenario: record task rejects a duplicate task name
  ...
  Scenario: record result generates a UUID in the id field
