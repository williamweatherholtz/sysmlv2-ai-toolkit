...
|----------------------------------------|-------------------------------------------|------------------------------------------------------|
| Ceremonies are actually run            | % sprints with all gates, in order?       | `validate_ceremony.py` + `sysmlv2 orient` in_progress |
| Needs are covered                      | % Needs with a satisfy edge?              | `traceability-audit` skill / `query.py trace-need`    |
