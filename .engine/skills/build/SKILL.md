...
name: build
description: |
  Compiles the deliverable (the Rust sysmlv2 workspace) and confirms the model
...
Covers the compile step of Delivery for the **deliverable** (the Rust `sysmlv2`
parser + CLI workspace). Output: a green release build of `sysmlv2.exe` and a
...
**Workspace layout:** Cargo workspace at repo root; crates `sysmlv2-parser` (lib)
and `sysmlv2-cli` (bin → `target/release/sysmlv2.exe`). Both compile under
...
3. **Confirm the binary exists:** `target/release/sysmlv2.exe`. Report its path.
4. **Validate the model still parses** with the no-kernel path:
   `./target/release/sysmlv2.exe validate .` (fast, no JVM). If the Rust
...
build:
  ...
  binary: target/release/sysmlv2.exe
model_validation:
