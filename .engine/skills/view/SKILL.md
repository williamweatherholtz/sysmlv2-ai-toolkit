...
by the Rust authority (`keel show view`), not per-view code (resolves issue018). Presentation is a
separate layer — the view's output is data (items + edges as JSON).

## Execute (x)
```
keel show view <name> [ROOT]      # reads .engine/views/<name>.view.toml, prints the subgraph JSON
```
...
- New recurring view → add a `.view.toml` here (don't write code); run it with `keel show view`.
