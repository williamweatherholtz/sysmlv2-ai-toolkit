...
| Track an issue | `keel record issue --title T --description-from FILE --severity S --resolver NAME [--in-field] --date D .` — the `#Resolves` edge is authored WITH the Issue; `keel guard issues` fails an untriaged one. Resolution is computed (D0077/D0078). |
...
| See sitting coverage | `keel show sitting-coverage` — which delivery sprints (`Story` items) have a covering per-sitting review vs await one (D0049/issue040). A "sitting review" attests its sprints via `#Covers` edges (review → sprint `Story`); a sprint is covered iff a `#Covers` edge points to it. A VIEW, not a gate — the human reviews per sitting at their own cadence (batchable, D0019). Recorded via the `sprint-review` skill (the human's explicit confirmation; never fabricated). `due` is the LIVE obligation — uncovered AND created after D0155's grandfather line; the sittings uncovered when that line landed are accepted-unreviewed by human attestation and reported separately as `grandfathered_unreviewed`, never deleted. If the line can't be resolved, nothing is grandfathered and every uncovered sitting is due, because an obligation surface must overstate rather than understate. |
| Inspect the backlog | `keel show outstanding` (not-done tasks); `keel item <task>` (done/deps/DoD/results); `keel show trace <task>` (transitive upstream + downstream over the succession DAG). |
...
| Add a new task | `keel add-task --file FILE --def DEF --task TASK --dod-from FILE [--method <VerificationMethod member>]` — members are the schema's (`inspect`, `analyze`, `demo`, `test`, `confirmation`, `critique`); auto UUID, duplicate names refused. |
...
| Run the forward guards | `keel guard [ROOT]` runs every enforced guard; `keel guard <name>` runs one. Catalogue: `.engine/docs/guards.md`; inventory: `keel version`. |
