...
| Add a new task | Use `keel add-task --file FILE --def DEF --task TASK --dod TEXT [--method <VerificationMethod member>]` — the members are the schema's (`inspect`, `analyze`, `demo`, `test`, `confirmation`, `critique` in `schema/core/element.sysml`; a refusal names them, issue451); auto-generates UUID, rejects duplicate names. |
