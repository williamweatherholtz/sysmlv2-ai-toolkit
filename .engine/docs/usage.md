...
| Run / define a viewpoint | `sysmlv2 view <name> [ROOT]` — executes `.engine/views/<name>.view.toml` (a select + optional traverse + project filter; D0075) and prints the subgraph JSON. Author/edit the `.view.toml` directly; see the `view` skill. Structural lenses are TOML views: `issues`, `decisions`, `process-changes` (M2.1, parity-verified vs query.py; the query.py equivalents are superseded, removed at M4). |
| Track an issue | Author a `part <name> : Issue { ... }` in `.tracking/issues.sysml` with `description`, `discoveredInField`, and `relatedTask` (the backlog action to address it). Surface with `sysmlv2 view issues` (the TOML view, M2.1; or the superseded `python .engine/tools/query.py issues`). |
...
| Check attestation coverage | Rust-native (no kernel): `sysmlv2 attestation-coverage [ROOT]` — or `python .engine/tools/query.py attestation-coverage`. Lists `status=accepted` decisions missing their acceptance event (the declared `attestation-coverage` viewpoint; M2.2a, parity-verified vs query.py). |
| Find orphaned / dangling items | Rust-native (no kernel): `sysmlv2 orphans [ROOT]` — or `python .engine/tools/query.py orphans`. Tasks with no `DoD`, Issues with no/dangling `relatedTask` (the `orphans` viewpoint; M2.2b, parity-verified vs query.py). |
| Audit sprint-process adherence | Rust-native (no kernel): `sysmlv2 audit [ROOT]` — or `python .engine/tools/query.py audit`. Charter coverage, ceremony completeness, estimation discipline, sitting-review currency, split ACTIONABLE vs grandfathered (D0046; M2.2b, parity-verified vs query.py). |
