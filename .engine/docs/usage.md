...
| Health/opportunity scorecards (reports) | `sysmlv2 report <assurance\|traceability\|quality-debt\|flow> [--html] [ROOT]` — computed AGGREGATE scorecards (D0087) rolling up the per-element views: **assurance** (verification coverage %, critique %, attestation %, open findings by severity, suspect load, READY/NOT) + **traceability** (% needs/requirements verified, satisfy/verify edge completeness — DO-178C-style) are HEALTH reads; **quality-debt** (charter debt, requirements volatility, suspect+stale) + **flow** (ready frontier, WIP, throughput, velocity, open issues) are OPPORTUNITY reads. `--html` emits a card scorecard. Grounded in the INCOSE DE Measurement Framework + SE Leading Indicators. A computed `#View` (git-ignored); the `report` viewpoint, deployed by the `report` skill. |
