...
| `claim-ancestry` | A claim's `claimedAt` cannot predate its own introducing commit by more than the expiry window (issue229: backdating steals holdership; D0013 applied to claims). Uncommitted claims skipped - they influence no other clone. Narrows the theft window from unbounded to the expiry span, stated plainly |
...
| `confirmation-authenticity` | That acceptance event is **human**-judged, never AI-fabricated (D0106/issue059), and — when the attestation policy declares a recording delegation (D0192 option A) — that a delegated acceptance actually quotes the human's words or cites their gesture, forward-only from the delegation date. Rule-sourced from `confirmationAuthenticityRule`, `delegatedAcceptanceSubstanceRule`, and — when the policy declares the confirmationRecord delegation (D0198) — `delegatedConfirmationSubstanceRule`: a human-judged confirmation FLIP after the cutoff must quote the human itself or carry a companion `<test>Attest<N>` quote receipt; a cited human gesture (deck/console/GitHub comment) also satisfies the acceptance check (issue235) |
...
| `claude-surface-drift` | The keel-owned subset of `.claude/` (six hook events, output style, per-registry skills) matches this binary's generator (D0174/P0.2) — the check IS `keel sync-claude --check`, one implementation. A project with no `.claude/` passes with zero scanned (CLI + commit/CI gates remain its enforcement). Version skew is a WARNING (regenerate obligation); drift in a keel-owned hook command is a violation — a mutated hook is a silently weakened control (K7). Foreign (user-owned) entries are never inspected — with ONE exception: `disableAllHooks` in `.claude/settings.json` or `settings.local.json` is a violation naming the file and the key, because Claude Code honours it from any scope and it silences every hook while the keel-owned entries stay byte-identical (issue365/D0296); `keel sync-claude` removes it, and `keel hook pre-write` DENIES a Write/Edit that sets it in every profile. The plugin rendering (`.engine/claude-plugin/`, `.claude-plugin/marketplace.json`, D0296) is checked byte-for-byte against the same generator. |
| `evidence-cited` | An AI-judged `method=test` result records WHAT WAS RUN, as a `// RAN:` receipt (D0232). Measured before building it: of 5,135 recorded results exactly ONE recorded what produced it, so every `pass` was a TESTIMONY nobody could re-derive - which is the mechanism behind a 3.9% fail rate. **AI-judged only, and that is the design rather than an exemption**: governance binds the AI, and a HUMAN's word IS the evidence, so demanding a receipt from them would point the control at the wrong party. `method=confirmation` is human by construction and never in scope. Forward-only from 2026-08-25 (the D0198 cutover shape): 983 test-results predate the convention and retro-fitting evidence nobody captured would mean inventing it. Tested in four directions including the human exemption. |
| `gating-workflow-history` | A workflow that RUNS the gate checks out full history (D0229). `claim-ancestry` and `audit-history` derive verdicts from git history and SKIP LOUDLY on a shallow clone - correctly, since a depth-dependent verdict is the machine-dependence K15 forbids - so a gating workflow that forgets `fetch-depth: 0` silently reports a pass it never verified. `ci.yml` carried the fix (issue229); `release.yml` never got it, and the release gate had been RED since guard 48 landed, undetected for weeks because releases are rare. Per-FILE not per-job: there is no YAML parser here, so a workflow whose non-gating job alone sets the flag would pass. |
| `process-applicability` | SCOPED TO ADOPTION (D0228): enforces nothing unless `.engine/processes/project-onboarding.sysml` is present, reporting 0 scanned when it is not - it failed the first adopting project 22 times out of 22 for a convention that project never chose, which is the D0164 failure. Where the process IS adopted, every file declaring a `Process` carries an `// APPLIES-WHEN:` line stating the SITUATION in which a project needs it (D0225). Onboarding recommends a process set by matching a project's elicited facts against these conditions, so a process that declares none is invisible to the match - recommendable neither for nor against - and the chartered set omits it silently, indistinguishably from a decision to omit it. Honest-state, not completeness: the guard does not require the set to be right, only that a process which exists can be reasoned about. The condition lives BESIDE the process rather than in a central table, because a central table cannot travel with one unit (issue253/D0222). |
...
| `manifest-key-portability` | A unit manifest's keys are portable: no absolute path, no machine-specific value, in any `.engine/contracts/*.toml` a downstream project will receive (issue301 class). A key that only resolves on the author's machine is a unit that only installs there |
| `control-map-reconciled` | `control-map.sysml` and the guard set agree both ways (D0195): every enforced guard is a `SystemSafetyConstraint` bound to a hazard, and every constraint names a guard that exists. A control the map does not know is unaccounted for; a mapped control that no longer runs is a claim |
| `sprint-closure` | A sprint whose CloseOut gate passes has every delivered task's DoD result recorded and every ceremony step before CloseOut passing; a closed sprint with an open task is a sprint closed on a count, not on work |
| `untrusted-routing` | An utterance whose `sourceTrust = untrusted` (a public GitHub issue, D0264) routes to a proposed Decision and never straight to work: any story descending from it that charters a task with no accepted Decision between is a violation. Enforces the ROUTING, never the judgment |
| `control-defect-registry` | Every entry in `.engine/contracts/control-defects.toml` (D0278) names an enforced control and an OPEN Issue with a valid direction (over/under); a registered defect whose Issue resolved is a stale claim that a green is not evidence, and an unknown control is a note nobody will see. An absent Issue is a warning (tracked upstream) |
| `decision-amends-process` | WARNING-tier (D0102 promote-once-low-noise; issue298/D0244): a staged Decision that names a process or one of its `ProcessStep`s by identifier while that process definition is unchanged in the same commit — if the Decision amends the step, the definition must carry it in the same commit ("a design may amend a process; it may not quietly disagree with one"); if it only cites the step, the warning is noise to read past. Measured before writing: 17 of 296 Decisions name a step, 5 without touching the file (D0242 a real amendment). STATED RESIDUAL: D0243's own sentence ("the source process's steps 1-2") names nothing and is NOT caught; step NUMBERS are not an anchor (two thirds of their mentions are not amendments). The durable fix for the unnamed shape is an `#Amends` edge — a schema change and a fork. |
| `unit-extras-present` | Every file a unit declares under `[unit] files` in `unit-extras.toml` exists in each project that has the unit installed (`installed-units.toml`); a missing one is a violation naming unit and path (D0300, issue290: penumbra adopted a unit whose definition and skill were present and whose four mechanism files were hand-staged out of the PR - nothing caught it, since `adoption-check` gates what EXPORT produces, not what a target received). A unit not installed here is not judged. Legitimately empty while no installed unit declares extras (the decision-channel unit, the only one that did, left at D0292). |
| `acceptance-binds-to-text` | HARD (D0308, issue341): an accepted Decision's `decision`, `rationale` and `consequences` at HEAD equal their text at the SHA its LATEST acceptance result binds to - the file at `judgedAgainst`, or the commit that introduced the file when the acceptance was recorded in the same commit (every standing-consent auto-accept). Compared as string VALUES (`+`-joined segments concatenated), so formatting is not drift. A difference names the Decision, both SHAs and the fields; the only remedy is `keel accept <d> --rebind --note "<what changed, why it still holds>"`, which appends `AcceptR<n+1>` bound to the current text - the first acceptance stands as when the Decision took effect (`governing-version` reads R1). First run: 17 of 301 had drifted, all editorially (issue371), re-bound not allowlisted. |
| `cli-surface-declared` | The CLI surface is an authored fact held equal to the dispatch and to the help, both ways (D0271, issue344): every command in `cli_surface::COMMAND_NAMES` and every lens in `LENS_NAMES` has a `CliCommand` in `.engine/cli/commands.sysml`, every fact is dispatched, and the Rust mirror (`cli_facts.rs`, which renders `keel --help`) matches the facts field by field. Armed both ways on landing: a flipped `effect` and a deleted fact each fail naming the command. Replaces the hand-written help catalogue, which had listed three `show` lenses as verbs for a month after D0273 moved them |

## Warning-only

| `decision-scaffolding` | An accepted `#ProspectiveChange` Decision must be reachable by a tracked-item edge (charteredby/derivedfrom/resolves/satisfy) — it promises process change, so it charters work (D0188, answering the human's "the scaffolding under a decision isn't being made"). WARNING-tier by D0188's composed rule with D0180: promotion to hard is a recorded review citing the fire-ledger window. Forward-only from D0188's own recorded acceptance date, read from the model; the newest violator is exempt (the landing-sprint grace). D0303 option C (issue331): a Decision created on/after 2026-09-05 whose text enumerates two or more clauses is a VIOLATION - one Decision per clause, `#DependsOn` edges between them - because the guard cannot see which clause an edge charters; earlier compound Decisions are grandfathered and counted in one warning. |
...
| `parser-coverage` | Statements the parser SKIPS are censused (any count warns), and with `.engine/contracts/parser-coverage-baseline.toml` declared the count EXCEEDING baseline is a violation - new invisibility goes red, legacy invisibility stays a visible warning, shrinkage prompts ratcheting the baseline down (issue231). Property-based skip-freedom over generated supported-form corpora backs it in keel-parser's test suite |
