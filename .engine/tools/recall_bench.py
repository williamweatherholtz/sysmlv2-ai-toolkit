...
WHY THERE IS A TWO-HOP ARM. `--hops 2` draws the query from a record two edges from the target, through
an intermediate whose words are ALSO removed. The default set cannot discriminate a mechanism that
reaches past one hop, because every one of its cases is a one-hop retrieval: measured, the
dominant-source second hop moved neither hits nor median position at any setting on the default set,
while on this one it took hits from 15/50 to 22/50. A criterion that asks for a two-hop mechanism to be
CHOSEN on the default set is asking a question that set cannot answer, and the answer to that is another
...
def main():
    ...
    def two_hop_sources(n):
        """Records exactly TWO edges from `n` that carry a body of their own - and the record between.

        WHY THIS SET EXISTS (recallRanksLinkedRecordsAsWellAsGrepDoes / issue367). Every case in the
        default set draws its query from a ONE-HOP neighbour, so a mechanism that reaches further is
        structurally invisible to it: measured, the dominant-source second hop moved neither hits nor
        median position at any setting on the 1-hop set, because the set contains no case it applies
        to. A criterion cannot ask for a two-hop mechanism to be CHOSEN on a set with no two-hop
        cases; this arm supplies them, generated by the same seeded, mechanical rule - never picked
        by hand and never the eight hand-written questions.
        """
        one = adj.get(n, set())
        out = {}
        for mid in sorted(one):
            for far in sorted(adj.get(mid, set())):
                if far != n and far not in one and far in with_body and far not in out:
                    out[far] = mid
        return out

    if args.hops == 2:
        two = {n: two_hop_sources(n) for n in sorted(with_body)}
        candidates = sorted(n for n in with_body if two[n])
    else:
        two = {}
    ...
    for i, name in enumerate(cases, 1):
        ...
        if args.self:
            ...
            # the intermediate's words are removed TOO: otherwise the query could name the very record
            # the traversal has to pass through, and the second hop would be handed its own stepping stone
            src = sorted(two[name])[0]
            mid = two[name][src]
            q = build_query(elements[src][1], name_words(name) | name_words(src) | name_words(mid))
        else:
