#!/usr/bin/env python3
"""Rewrite every call site of the five folded channel verbs to its `keel github <sub-verb>` spelling —
the D0453 fold of the channel family, the fifth and last of the folds d0399 option A named.

D0453: the five top-level names are REMOVED from the dispatch and one router resolves them as
sub-verbs, in the sprint 512 shape (D0273): a committed transform, a dry run that reconciles control
totals, call-site rewrites in the SAME commit as the removal, no alias window. Each verb keeps the
word after its hyphen and its arguments, so every argument rides along unchanged and the sprint
compares the pre-fold and post-fold binaries byte for byte on stdout, stderr and exit code over the
fixtures the Rust tests use. `keel currency` keeps its name (D0338) and is not a site. The mapping is
the Decision's:

    keel github-pull ...         ->  keel github pull ...
    keel github-ingest ...       ->  keel github ingest ...
    keel github-decider ...      ->  keel github decider ...
    keel github-gesture ...      ->  keel github gesture ...
    keel github-decision-id ...  ->  keel github decision-id ...
...
`${KEEL}`, and the python list shape `[KEEL, "github-pull", ...]` the brief scripts use.
...
WHAT IT DELIBERATELY DOES NOT TOUCH (the D0273 transform's rule, unchanged).
  ...
  - Rust ARGV literals (`["github-ingest", ...]`): code, rewritten by hand with the router itself. The
  ...
  - A verb spelled BARE, with no binary in front (`the github-ingest dispatch arm`, a refusal
    message's `github-pull:` prefix): the rule reads `keel <verb>`; the sprint sweeps those by hand
    from the census and the retro records what the sweep found.

Idempotent by construction: the pattern requires the binary immediately followed by one of the five
old verbs, and `github` is not one of them, so `keel github pull` is never rewritten again; the
lookahead `(?![\\w-])` keeps `keel github-decision` from matching inside `keel github-decision-id`.

Usage:
    python .engine/tools/migrations/2026-09-12-channel-family-fold.py --dry-run
    python .engine/tools/migrations/2026-09-12-channel-family-fold.py --apply
"""
import io
import os
import re
...
TABLE = {
    ...
    "github-decider": ("github", "decider"),
    "github-gesture": ("github", "gesture"),
    "github-decision-id": ("github", "decision-id"),
}
...
# The python list shape: `KEEL, "github-pull"` (the brief scripts build argv lists from a KEEL constant).
...
def main(argv):
    ...
    print("channel-family-fold: %d call site(s) across %d file(s)" % (sites, files))
    for v in TABLE:
        if per_verb.get(v):
