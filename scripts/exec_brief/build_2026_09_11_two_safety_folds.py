...
"""Build the standing decision brief for the 2026-09-11 (sixteenth) queue change: the thirty-five asks of
the fifteenth publish were all answered and the queue now holds the two safety Decisions the chosen verb
programme reserved for the human's own word - the gating family folding into gate and audit, and the
github verbs folding into github. Neither is a fork: each is accepted, held or rejected with a word. One
tab per ask (D0404); the members of each fold are named (D0406); the call-site counts by area are the
census facts.py computes with the method the Decisions' research names, never typed.

Usage: python scripts/exec_brief/build_2026_09_11_two_safety_folds.py <facts.json> <previous.html> <out.html>
"""
import re
import sys
...
# ---- the queue: exactly the two safety Decisions, neither a fork -------------------------------------
...
if [p["slug"] for p in members] != ["d0452", "d0453"]:
    sys.exit(f"refusing: this page is written for the gating and channel folds; the queue is {[p['slug'] for p in members]}")
...
<div class="ask"><p class="verdict"><strong>Two asks</strong> from the programme you chose, each held for your own word. <strong>Gating:</strong> ten names go behind gate and audit, hooks and CI in the same commit. <strong>Github:</strong> five names go behind github; currency stays. <strong>Accept both</strong>, or hold either. <strong>Wrong if</strong> a hook must keep a bare name.</p></div>
<div class="chips"><span class="chip"><b>Waiting on you</b> {pending}</span><span class="chip"><b>Dispatch arms</b> {arms} &rarr; {arms_after}</span></div>
...
<footer data-digest="provenance">Computed from the repository at {TREE} on {DATE}; {dirty} files carried uncommitted edits. Call sites: grep for keel plus a family verb over tracked files, by area. {tests} tests, {failing} failing. <a href="https://github.com/williamweatherholtz/sysmlv2-ai-toolkit" target="_blank" rel="noopener">repository</a>.</footer>
</div>
"""


TITLE = "Accept both safety folds: every gating and github verb keeps its word behind one router"
SUB = "Two asks"
...
print(f"wrote {out_path}: {len(page)} bytes; frame {frame_words} words; panels " +
      ...
      f"; summed {words(page)}; members tables name {len(TWELVE)} gating and {len(C_MOVE) + len(C_STAY)} channel verbs"
      f" ({len(G_MOVE)} and {len(C_MOVE)} names removed; {len(C_DEPRECATED)} of the channel's already deprecated)")
